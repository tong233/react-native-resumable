...
/*
 * MIT Licensed
 * 兼容RN:主要是将File改成Blob,增加allowDuplicateUploads属性,发送切片后进行close(),去除web端拖拽上传的代码
  ...
  var Resumable = function(opts) {
    ...
    var appendFilesFromFileList = function(fileList, event) {
      ...
      $h.each(fileList, function(file) {
        ...
        function addFile(uniqueIdentifier) {
          if ($.getOpt('allowDuplicateUploads') || !$.getFromUniqueIdentifier(uniqueIdentifier)) {
            (function() {
    ...
    function ResumableChunk(resumableObj, fileObj, offset, callback) {
      ...
      $.send = function() {
        ...
        if ($.getOpt('chunkFormat') === 'blob') {
          $.xhr.send(data);
          data.close(); // 关闭Blob对象，释放资源，防止内存溢出
        }
      };
      $.abort = function() {
        // Abort and reset
